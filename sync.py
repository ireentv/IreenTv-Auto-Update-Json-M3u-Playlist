...
def main():
    ...
    for item in config_data.get("playlists", []):
        ...
        for ch in channels:
            # We construct a clean dict in the exact desired key order:
            # name, logo, url, group, url_raw, headers, attrs, vlc_opts, kodiprops, status
            clean_ch = {
                "name": ch.get("name", ""),
                "logo": ch.get("logo", ""),
                "url": ch.get("url", ""),
            ...
            # We explicitly exclude exthttps to keep JSON output clean, as requested
